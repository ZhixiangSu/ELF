...
void GraphUtil::topo_leveler(Graph& g){
	...
	for (int i=0; i < N ; i++)
			topo_level(g, i);
	///
	g.set_in_out_choice(false);
	/*if (top_level_count[1] > top_level_count[0] * 0.1)
	{
		g.set_in_out_choice(false);
	}
	else
	{
		g.set_in_out_choice(true);
	}*/
	/// 
}
...
///
void GraphUtil::ELF_construction(Graph& g, vector<int>& reverse_topo_sort)
{
	if (g.get_in_out_choice())
	{
		for (int u = 0; u < g.num_vertices(); u++) {
			g[u].max_node = -g.in_degree(u) * g.num_vertices() - u;
		}
	}
	else
	{
		for (int u = 0; u < g.num_vertices(); u++) {
			g[u].max_node = -g.out_degree(u) * g.num_vertices() + u;
		}
	}
	for (int i = 0; i < reverse_topo_sort.size(); i++) {
		int u = reverse_topo_sort[i];
		int tmp_max_node = g[u].max_node;
		for (int j = 0; j < g.out_degree(u); j++) {
			int addr = g.out_edges(u)[j];
			if (g[addr].max_node > tmp_max_node) {
				tmp_max_node = g[addr].max_node;
			}
		}
		g[u].max_node = tmp_max_node;
	}
}
/// 