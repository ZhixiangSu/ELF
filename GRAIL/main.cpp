...
int main(int argc, char* argv[]) {
	...
	string dataname = "Email-EuAll";
	string file = "../../../data/" + dataname + "_dag_uniq.gra";
	string query = "../../../query/" + dataname + "/query.txt";
	filename = new char[100];
	testfilename = new char[100];
	strcpy(filename, file.c_str());
	strcpy(testfilename,query.c_str());
	alg_type = 2;
	/// 
	...
	*/
	...
	/// 
	GraphUtil::topo_leveler(g);
	GraphUtil::ELF_construction(g, reverse_topo_sort);
	/// 
...
//		cout << "UP TO HERE OK" << endl;
	...
	cout << "Labeling_time = " << labeling_time + exceptionlist_time << "\t Query_Time = " << query_time << "\t Index_Size = " << float(gsize * DIM * 2) / 1024 << "\t Mem_Usage = " << print_mem_usage() << " MB" << endl;
	cout << float(gsize * DIM * 2 + gsize) / 1024 << endl;
	// process queries
