...
bool Index::reachable(unsigned x, unsigned y) {
  ...
  if (
      ...
      ///
      tlevel_[x] <= tlevel_[y] || torder_[x] > torder_[y]
...
void Index::ELF_construction()
{
    ...
    if (in_out_choice)
    {
        std::vector<unsigned> indeg = *(g->get_indegrees());
        for (int u = 0; u < n_; u++) {
            max_node[u] = -indeg[u] * n_ - u;
        }
    }
    else
    {
        for (int u = 0; u < n_; u++) {
            max_node[u] = -deg[u] * n_ + u;
        }
    }
    const std::vector<unsigned>* nb;
    for (int i = ordered_nodes_.size() - 1; i >= 0; i--) {
        int u = ordered_nodes_[i];
        int tmp_max_node = max_node[u];
        nb = g->get_neighbors(u);
        for (std::vector<unsigned>::const_iterator it = nb->begin(); it != nb->end();
            ++it) {
            int addr = *it;
            if (max_node[addr] > tmp_max_node) {
                tmp_max_node = max_node[addr];
            }
        }
        max_node[u] = tmp_max_node;
    }
}
/// 