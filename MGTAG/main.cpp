...
void read_graph(const char *filename) {
  ...
  for (int u = 0; u < n; u++) {
	...
	///make a backup for indegree.
	nodes[u].indegree2 = nodes[u].indegree;
	/// 
...
/// choose indegree or outdegree to sort by
bool in_out_choice;
int level_count[2];
/// 
void cal_index(){
...
void ELF_construction()
{
	...
	if (in_out_choice)
	{
		for (int u = 0; u < nodes.size(); u++) {
			nodeLabel1[u].max_node = -nodes[u].indegree2 * nodes.size() +u;
		}
	}
	else
	{
		for (int u = 0; u < nodes.size(); u++) {
			nodeLabel1[u].max_node = -nodes[u].N_O_SZ * nodes.size() - u;
		}
	}
	for (int i = TopoOrder.size() - 1; i >= 0; i--) {
		int u = TopoOrder[i];
		int tmp_max_node = nodeLabel1[u].max_node;
		for (int j = 0; j < nodes[u].N_O_SZ; j++) {
			int addr = nodes[u].N_O[j];
			int addr_newID = nodeLabel1[addr].oldID_mapto_newID;
			if (nodeLabel1[addr].max_node > tmp_max_node) {
				tmp_max_node = nodeLabel1[addr].max_node;
			}
		}
		nodeLabel1[u].max_node = tmp_max_node;
	}
}
/// 
...
void run_queries() {
	...
	/////////////////
	/////MGTAG query
	///// calculate fitered nodes number
	//int filtered_num = 0;
	/////
	...
	///// 
	//cout << endl;
	//cout << "MGTAG result:" << endl;
	//cout << "unfiltered_num:" << src.size() - filtered_num << endl;
	/////
	...
	int filtered_num2 = 0;
	///
	int reachable2 = 0, nonreachable2 = 0;
	int falsepositive2 = 0, falsenegative2 = 0;
	gettimeofday(&start_at, 0);
	...
	for (vector<int>::iterator x = src.begin(), y = dest.begin(); x != src.end(); x++, y++) {
		if (*x == *y)
			...
			nodeLabel1[*x].Level.first <= nodeLabel1[*y].Level.first
			/// 
			|| nodeLabel1[*x].max_node < nodeLabel1[*y].max_node
			/// 
	...
	/// 
	cout << endl;
	cout << "MGTAG with ELF result:" << endl;
	cout << "unfiltered_num:" << src.size()-filtered_num2 << endl;
	///
	cout << "reachable: " << reachable2 << "  nonreach: " << nonreachable2 << endl;
	cout << "falsepositive: " << falsepositive2 << "falsenegative: " << falsenegative2 << endl;
	cout << "traverse num: " << traverseCnt << " average: " << (double)traverseCnt / (1000000) << endl;
	printf("query time: %.3fms\n",
		(end_at.tv_sec - start_at.tv_sec) * 1000 +
		double(end_at.tv_usec - start_at.tv_usec) / 1000);
